...
def generate_text_report(
    ...
    *,
    ...
    """Write a plain-text report for easy reading in any editor."""
    now = datetime.now(timezone.utc)
    generated_at = now.strftime("%Y-%m-%d %H:%M UTC")
    lines = [
        "=" * 72,
        ...
        f"Sources used: {', '.join(sources_used) if sources_used else 'RSS'}",
    ...
    for story in stories:
        ...
        wrapped_summary = textwrap.fill(summary, width=68, initial_indent="  ", subsequent_indent="  ")

        lines.extend(
            [
                f"#{rank}  [{sc} source{'s' if sc != 1 else ''}] {title}",
                f"  First published: {first_pub}",
                f"  Covered by: {sources}",
                f"  Link: {url}",
                "  Summary:",
                wrapped_summary,
                "",
        ...
        if story.get("hook"):
            lines.append(f"  Reel hook: {story['hook']}")
        lines.append("")

    lines.append("=" * 72)
    lines.append("Generated by AI News Finder")
